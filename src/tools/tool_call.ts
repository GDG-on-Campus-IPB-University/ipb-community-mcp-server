...
export async function handleToolCall(name: string, args: any): Promise<CallToolResult> {
    switch (name) {
        ...
        case "get-gdgoc-ipb-community-check-member":
            try {
            let data = await gdgoc_ipb_community_service_check_member(
                args.instagram, args.nama_lengkap, args.asal_universitas, args.fakultas, args.email);
            return {
                content: [{
                type: "text",
                text: JSON.stringify(data, null, 2),
        ...
        case "get-gdgoc-ipb-community-add-member":
            try {
                let data = await gdgoc_ipb_community_service_add_member(
                    args.instagram, args.linkedin, args.nama_lengkap, args.gdg_profile, args.student_number, args.asal_universitas, args.fakultas, args.program_studi_departemen, args.tahun_angkatan, args.gdg_join, args.other_community, args.motivasi, args.harapan, args.kontribusi, args.toc, args.email
                );
                return {
                content: [{
                    type: "text",
        ...
        case "get-ipb-integration-rag":
            try {
                let data = await ipb_integration_rag(
                args.query);
                return {
                content: [{
                    type: "text",
