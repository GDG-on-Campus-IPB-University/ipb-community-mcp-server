...
import { makeGDGOCAPIRequest, makeSimpleGETFetchRequest } from "../../utils/apirequest.js";

export let gdgoc_ipb_event_informations_tool: Tool[] = [
    {
...
export let gdgoc_ipb_event_informations = async (): Promise<CallToolResult> => {
...
function formatEvent(event: EventResult): string {
    return [
...
async function fetchAndFormatEvents(apiUrl: string): Promise<string[] | null> {
    const eventResponse = await makeSimpleGETFetchRequest<EventApiResponse<EventResult>>(apiUrl);
